...
export class URLValidator {
  ...
  static filterSupportedUrls(urls: string[]): { supported: string[]; unsupported: string[] } {
    console.log('URLValidator.filterSupportedUrls called with:', urls);
    ...
    urls.forEach(url => {
      ...
      if (this.isValidUrl(trimmedUrl) && this.isSupported(trimmedUrl)) {
        console.log('URL is supported:', trimmedUrl);
        supported.push(trimmedUrl);
      } else {
        console.log('URL is not supported:', trimmedUrl, {
          isValid: this.isValidUrl(trimmedUrl),
          isSupported: this.isSupported(trimmedUrl)
        });
        unsupported.push(trimmedUrl);
      }
    });
    
    console.log('Filter result - Supported:', supported, 'Unsupported:', unsupported);
    return { supported, unsupported };
  }
}
